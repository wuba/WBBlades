...
- (NSString *)formattedString:(NSString *)previousName formatter:(CDTypeFormatter *)typeFormatter level:(NSUInteger)level;
{
    ...
    switch (self.primitiveType) {
        ...
        case '{':
            ...
            if (baseType == nil) {
                if (_typeName == nil || [@"?" isEqual:[_typeName description]]) {
                    _typeName.name = [self unkonwTypeMaps:self];
                }
...
- (NSString *)formattedStringForSimpleType;
{
    ...
    switch (_primitiveType) {
        ...
        case 'B': return @"BOOL"; // C99 _Bool or C++ bool
        case 'v': return @"void";
        case '*': return @"STR";
        case '#': return @"Class";
        case ':': return @"SEL";
...
- (void)phase0RecursivelyFixStructureNames:(BOOL)flag;
{
    ...
    int variableCount = 0;
    for (CDType *member in _members) {
        if (0 == member.variableName.length) {
            member.variableName = [NSString stringWithFormat:@"var%@", @(variableCount++)];
        }
...
- (NSString *)unkonwTypeMaps:(CDType *)type  {

    NSDictionary *typeMaps = @{@"dddd": @"CGRect", @"cccc":@"CGColor", @"dd":@"CGSize", @"ddd":@"CMAcceleration", @"CCCC":@"CGColorRef", @"ffff":@"CGSize"};
//    ffff
//    ii
    if(type.members.count > 0) {
        ...
        for (int i = 0; i < type.members.count; i++) {
            char typeChar[2];
            typeChar[0] = ((CDType *)type.members[i]).primitiveType;
            typeChar[1] = '\0';
            NSString *typeStr = @(typeChar);
            if(typeStr) {
