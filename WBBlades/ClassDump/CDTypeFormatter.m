...
- (NSString *)formatCaptainMethodName:(NSString *)methodName typeString:(NSString *)typeString parserComplete:(void (^)(NSString *returnType, NSArray<NSString *> *methodLabels, NSArray <NSString *> *methodLabelTypes))compelteBlock {
    NSString *returnType;
    NSMutableArray *methodLabels = [@[] mutableCopy];
    NSMutableArray *methodLabelTypes = [@[] mutableCopy];
    ...
    NSMutableString *resultString = [NSMutableString string];
    {
        ...
        if ([methodType.type.typeString  hasPrefix:@"^{?="] || [methodType.type.typeString  hasPrefix:@"{?="])  {
            NSString *anonymousStructName = [WBBladesScanManager anonymousWithType:methodType.type];
            if (anonymousStructName.length > 0) {
                returnType = anonymousStructName;
            }
        }
        ...
        while ([scanner isAtEnd] == NO) {
            ...
            if ([scanner scanString:@":" intoString:NULL]) {
                if (index >= count) {
                    ...
                    methodType = methodTypes[index];
                    specialCase = [self _specialCaseVariable:nil type:methodType.type.bareTypeString];
                    NSString *methodLabelType = nil;
                    if (specialCase != nil) {
                        methodLabelType = [NSString stringWithFormat:@"%@", specialCase];
                    } else {
                        NSString *formattedType = [methodType.type formattedString:nil formatter:self level:0];
                        //if ([[methodType type] isIDType] == NO)
                        methodLabelType = [NSString stringWithFormat:@"%@", formattedType];
                        if ([methodType.type.typeString  hasPrefix:@"^{?="] || [methodType.type.typeString  hasPrefix:@"{?="]) {
                            NSString *anonymousStructName = [WBBladesScanManager anonymousWithType:methodType.type];
                            if (anonymousStructName.length > 0) {
                                methodLabelType = [NSString stringWithFormat:@"struct %@", anonymousStructName];
                            }
                        }
                    }
                    [methodLabelTypes addObject:methodLabelType ?: @""];
...
//        }
     ...
     compelteBlock(returnType, methodLabels, methodLabelTypes);
    return nil;
}
