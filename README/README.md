...
## 背景
   ...
   WBBlades工具集的技术原理是对Mach-O文件的解析。整个项目的核心是基于类是如何在Mach-O文件中存储的。在了解类在Mach-O文件中存储结构后，我们就能通过Mach-O文件获取当前项目中有哪些类，每个类的类名及方法名，以及方法的指令存储范围，从而能够完成仿照链接器对静态库二进制进行数据提取和合并、获得静态库链接后大小，检测项目中没有用到的类，以及寻找崩溃偏移地址位于哪个函数的指令区间范围的任务。
...
![linkmap VS WBBlades](linkmapVSWBBlades.png)

准确性：分析结果与linkmap十分接近，偏差在10%以内。
![linkmap VS WBBlades on result data](sizeData.png)
上图为随机选取的19个SDK，分别采用linkmap和WBBlades工具分析出的结果。

* **无用类检测 VS otool+Mach-O**

目前市面上使用较多的方案是基于otool+Mach-O进行检测，但对于像58同城这样体积庞大、内部代码复杂的APP，采用此方案解析出的数据误差较大，会将很大一部分存在调用的类检测为无用类。

![otool VS WBBlades on result data](otool.png)

耗时问题：我们提供了圈选功能，开发者可以查看某几个SDK或某几个Pod的无用类，从而减少耗时。

* **日志解析工具**

WBBlades的日志解析功能最大的特点是不依赖任何符号表。以58同城为例，为了能够尽可能的减小下载体积，58同城将APP的符号表在打包期间从应用程序中剥离出来形成dSYM文件，保存在打包服务器中，测试同学下载的测试包是不包含符号表信息的。由于剥离出来的dSYM文件较大，为了节省服务器空间，dSYM在保留2天后会自动清除。假设有这样一个场景，即测试同学下载了一个测试包，在测试到第三天时发生了不可稳定复现的崩溃，在这种即没有dSYM文件也没有bugly的symbol文件的情况下，如何才能恢复堆栈符号？为此，我们通过解析Mach-O文件，查找崩溃偏移地址位于哪个函数的指令区间范围内，从而实现崩溃日志的符号化。

除此之外，WBBlades的日志解析功能可以集成到APP中，在APP内实现崩溃拦截+崩溃解析的一条龙服务。
...
2. **无用类检测**：
   ...
   * 使用命令行
   ...
   在拖入的WBBlades文件路径后面加空格，输入数字2，空格，拖入需要解析的app文件路径——最好是Xcode本地打出来的Debug包；如果只需要检测部分静态库，则拖入需要解析的静态库路径，可以拖入一个到多个，将只检测这些静态库下的类的使用情况；各参数间以空格隔开。
